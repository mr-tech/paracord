  ...
  public handleEvent(eventType: ParacordGatewayEvent | GatewayEvent | ParacordEvent, data: unknown, gateway: Gateway): void {
    switch (eventType) {
      case 'READY':
        ...
        break;
      case 'RESUMED':
        // A resumed gateway always completes shard startup, whether or not it is the
        // current `#startingGateway` — `clearStartingShardState` (called via
        // `completeShardStartup`) is a no-op for any other gateway, so this only ever
        // clears state, timers included, for the gateway that just resumed.
        markReady(gateway);
  ...
  private processGatewayQueue = async (): Promise<void> => {
    ...
    try {
      ...
      if (!this.#startingGateway) {
        // get resumable shard
        this.#startingGateway = this.gatewayLoginQueue.find((g) => g.resumable && isEligible(g, now));
      }
      ...
      if (!this.#startingGateway && this.#allowConnection) {
        ...
        for (const gateway of queue) {
          if (isEligible(gateway, now) && await this.#allowConnection(gateway)) {
