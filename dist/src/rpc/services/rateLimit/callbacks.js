...
const { BaseRequest, RateLimitHeaders, } = require('../../../clients/Api/structures');
...
module.exports = function (server, cache) {
    ...
    function update(call, callback) {
        try {
            const { requestMeta, global, bucket, limit, remaining, resetAfter, } = RateLimitStateMessage.fromProto(call.request);
            ...
            if (bucket === undefined) {
                ...
                const rateLimitHeaders = new RateLimitHeaders(global, bucket, limit, remaining, resetAfter);
