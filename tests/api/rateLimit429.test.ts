...
/**
 * WP-9b step 1 — AC-9.1 (send counts) and AC-9.2 (client-side recovery), over the 429
 * shape class × the request path {local, RPC} (D-20). Header/body values for the
 ...
 * Cloudflare shape (shortened here to a value still outside the 4.5 s window, so the
 * test does not itself run for over ten minutes — the *shape* under test is the same:
 * a long, HTML-bodied, header-only park).
 *
 * The information-free shape (WP-9b step 6, D-17) is added once that step lands — its
 * count depends on the growth schedule this file's sibling step does not yet build.
 *
 * "Fail at WP-9b's parent, pass after the fix" per plan Testing Strategy: these
 * properties held unchanged since WP-9a (`f184b07`) through this package's parent —
 * re-derived directly against the pre-WP-9b tree in this session's TDD gate rather than
 * an isolated clone at that SHA (Testing Strategy: "the means are QA's" — the isolated
 * historical clone re-derivation is QA's to execute; this suite states the property).
 */
